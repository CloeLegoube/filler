...
int		ft_reduce_piece_line(t_game *game, char *lines, int count)
{
	int		i;
	int		j;
	int		k;
...
void	ft_calcul_subline(t_game *game)
{
	...
	int		count;
	int		start;
	char	lines[game->piece_line];

	i = -1;
	count = 0;
	ft_bzero(lines, game->piece_line);
	dprintf(2, "helllo\n");
	while (++i < game->piece_line)
	{
		j = 0;
	...
	dprintf(2, "AVANT lines[%i] %d - %d - count: %d\n", i, lines[i], lines[i + 1] - 1, count);

	if (lines[0] == '0')
	{
		i = 0;
		count = 1;
		while (lines[i] + 48 == lines[i + 1] + 48 - 1)
		{
...
int		ft_reduce_piece_col(t_game *game, char *cols, int count)
{
	int		i;
	int		j;
	int		k;
...
void	ft_calcul_subcol(t_game *game)
{
	...
	int		count;
	int		start;
	char	cols[game->piece_col];

	count = 0;
	j = -1;
	start = -1;
	ft_bzero(cols, game->piece_col);
	...
	dprintf(2, "AVANT cols[%i] %d - %d  - count: %d\n", i, cols[i], cols[i + 1] - 1, count);

	if (cols[0] == '0')
	{
		i = 0;
		count = 1;
		while (cols[i] + 48 == cols[i + 1] + 48 - 1)
		{
